...
from nano_gpt import SSM_GPT2Model, GPT2Config, LayerNorm
...
class SSM_TransformerModel(nn.Module):
...
class SSM_TransformerModelLooped(SSM_TransformerModel):
    def __init__(
            self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4, loop_func='z=f(x+z)', pred_type='regression'):

        super(SSM_TransformerModelLooped, self).__init__(
